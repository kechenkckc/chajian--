...
const FIELD_ALIASES = {
  ...
  "博主类型": ["博主类型", "达人主类型", "账号主类型", "内容主类型", "主要内容形式", "主要笔记形式", "主发形式", "主发类型", "作品形式", "发布形式", "图文/视频", "图文或视频", "creator_type", "blogger_type", "primary_note_type", "primary_content_type"],
  ...
  "账号类型": ["账号类型", "达人类型", "达人类目", "博主类目", "内容类型", "内容类目", "蒲公英类目", "官方类目", "博主标签", "账号标签", "内容标签", "creator_category", "categoryName", "category", "contentTags", "tradeType", "industryTag", "type"],
  "自定义标签": ["达人标签", "自定义标签", "标签", "用户标签", "customTags", "custom_tags"],
...
function normalizeExportRow(row) {
  ...
  const creatorCategory = compactTagText(valueByAliases(row, "账号类型") || rawValueByKeys(row, ["creator_category", "contentTags", "tradeType", "industryTag", "type"]) || legacyCreatorCategory);
  const output = {
...
function semanticCanonicalFieldForHeader(header, context = "") {
  ...
  if (leafHas(/^(内容形式|笔记类型|笔记形式)$/i)) return "笔记类型";
  const creatorTypeSubject = has(/博主|达人|账号|作者|创作者|creator|blogger|kol/i);
  const creatorTypeMeaning = has(/主类型|主要类型|主发|主做|主攻|主要内容形式|主要笔记形式|作品形式|发布形式|媒介形式|图文.?视频|图文或视频|图文还是视频|primarycontenttype|primarynotetype/i);
  ...
  if (leafHas(/^(达人类型|达人类目|内容类型|内容类目)$/i)) return "账号类型";
  if (leafHas(/^博主类型$/i)) return "博主类型";
...
function canonicalFieldForHeader(header, context = "") {
  ...
  if (/^(达人类型|达人类目|内容类型|内容类目)$/i.test(headerText.replace(/\s+/g, ""))) return "账号类型";
  if (/^博主类型$/i.test(headerText.replace(/\s+/g, ""))) return "博主类型";
...
function isAmbiguousCreatorTypeHeader(header) {
  return /^博主类型$/i.test(cellText(header).replace(/\s+/g, ""));
}
...
function canonicalFieldForSheetColumn(header, context, values, columnIndex, dataStartIndex) {
  ...
  if (canonicalField === "账号类型" || canonicalField === "笔记类型") return canonicalField;
...
function hasBlankMappedDetailCell(item, columns) {
  ...
  return columns.some((column) => {
    ...
    if (column.canonicalField === "账号类型" && normalizedNoteTypeDisplay(item.line[column.columnIndex])) return true;
...
function bitableCategoryFieldName(fields) {
  const candidates = (fields || []).filter((field) => canonicalFieldForHeader(bitableFieldName(field)) === "账号类型");
  const preferred = candidates.find((field) => /^(达人类型|达人类目|内容类目)$/i.test(bitableFieldName(field)));
  return bitableFieldName(preferred || candidates[0]) || "账号类型";
}

function bitableNoteTypeFieldName(fields) {
  const candidates = (fields || []).filter((field) => canonicalFieldForHeader(bitableFieldName(field)) === "笔记类型");
  const preferred = candidates.find((field) => /^(内容形式|笔记类型|笔记形式)$/i.test(bitableFieldName(field)));
  return bitableFieldName(preferred || candidates[0]) || "笔记类型";
}
...
async function ensureBitableDetailFields(token, appToken, tableId, records = []) {
  ...
  const detailFieldNames = {
    "笔记类型": bitableNoteTypeFieldName(fields),
    "账号类型": bitableCategoryFieldName(fields)
  };
  for (const fieldName of DETAIL_BACKFILL_FIELDS) {
...
function bitableNeedsDetail(item, fieldsMeta = [], options = {}) {
  ...
  const categoryValue = item.row[bitableDetailFieldName(fieldsMeta, "账号类型")];
  if (normalizedNoteTypeDisplay(categoryValue)) return true;
  return DETAIL_BACKFILL_FIELDS.some((fieldName) => !nonEmptyCell(item.row[bitableDetailFieldName(fieldsMeta, fieldName)]));
}
...
function detailValuesForSheet(detail, captures, status, note = "") {
  ...
  return {
    ...
    "笔记类型": normalizedNoteTypeDisplay(valueByAliases(detail, "笔记类型")) || fallbackNoteTypeValue(detail) || creatorTypeFromDetail(detail),
...
function preFavoritePatchFromApiCache(userId, apiCache, { markDataRefreshed = false } = {}) {
  ...
  if (categoryTags.length) {
    patch.categoryTags = categoryTags;
    patch.categoryText = categoryTags.join("；");
    patch.categorySource = "pgy_profile";
  }
  const contentForm = normalizedNoteTypeDisplay(detail.note_type) || creatorTypeFromDetail(detail);
  if (contentForm) patch.contentForm = contentForm;
  ...
  try {
    ...
    try {
      ...
      for (const column of shape.columns) {
        ...
        const refreshLegacyCategory = column.canonicalField === "账号类型" && Boolean(normalizedNoteTypeDisplay(current));
        if (nonEmptyCell(current) && !refreshLegacyCategory) continue;
        if (!column.canonicalField) continue;
